...
<template>
  <section class="projects-section" :class="state.theme">
    <h4 class="projects-title" :class="state.theme">
      Последние проекты
      <span>{{ projects.length }}</span>
    </h4>

    <ul class="projects-list" :class="state.theme">
      <!-- todo: сделать функцию для сортировки по строке с датой -->
      <li
        v-for="project in projects" :key="project.started"
        class="project-container" :class="state.theme"
      >
        <img v-if="project.img" :src="project.img" alt="" class="icon" />
        <div v-else class="icon placeholder" />

        <div class="project-wrapper" :class="state.theme">
          <h5>{{ project.name }}</h5>
          <small>
            Начало работ:&#32;
